...
export type StatKey = "total" | "occupied" | "available" | "rate";

interface Props {
  stats: StatsData;
  hidden?: StatKey[]; // 👈 which stats to hide
}

export const ParkingStats = ({ stats, hidden = [] }: Props) => {
  ...
  const visibleStats = statItems.filter((item) => !hidden.includes(item.key));

  return (
    <>
      {visibleStats.map((item) => (
        <div key={item.key} className={styles.stat}>
          <div className={`${styles.stat__icon} ${item.colorClass}`}>
            {item.icon}
          </div>
